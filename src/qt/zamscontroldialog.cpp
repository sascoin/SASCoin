// Copyright (c) 2017 The SasCoin developers
...
std::list<std::string> ZSASControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZSASControlDialog::listMints;

ZSASControlDialog::ZSASControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZSASControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZSASControlDialog::~ZSASControlDialog()
{
    delete ui;
}

void ZSASControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSASControlDialog::updateList()
{
...
void ZSASControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSASControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZSAS_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSASControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZSASControlDialog::GetSelectedMints()
{
...
void ZSASControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
