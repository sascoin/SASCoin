...
#if defined(NDEBUG)
#error "SasCoin cannot be compiled without assertions."
#endif
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from SasCoin
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    bool fEnforceBIP30 = (!pindex->phashBlock) || // Enforce on CreateNewBlock invocations which don't have a hash.
                         !((pindex->nHeight == 300 && pindex->GetBlockHash() == uint256("0x001")) ||
                             (pindex->nHeight == 308 && pindex->GetBlockHash() == uint256("0x001")));
    if (fEnforceBIP30) {
        BOOST_FOREACH (const CTransaction& tx, block.vtx) {
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    if (block.GetBlockTime() > Params().Zerocoin_StartTime()) {
        if(block.nVersion < Params().Zerocoin_HeaderVersion())
            return state.DoS(50, error("CheckBlockHeader() : block version must be above 8 after ZerocoinStartTime"),
            REJECT_INVALID, "block-version");
    } else {
        if (block.nVersion >= Params().Zerocoin_HeaderVersion())
            return state.DoS(50, error("CheckBlockHeader() : block version must be below 8 before Version"),
            REJECT_INVALID, "block-version");
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // SasCoin: We use certain sporks during IBD, so check to see if they are
